using ModelingToolkit, DataFrames, RigidBodyDynamics, DifferentialEquations
using ModelingToolkitStandardLibrary.Blocks: RealOutput, RealInput
...
@component function VariableInput(; name, experiment = 5)
...
@component function VariableInputV2(; name, experiment = 5)
    @named output = RealOutput()
    u_function = u_functions[experiment]
    equation = u_function(t)
...
@mtkmodel OLController begin
    ...
    @components begin
        ...
        joint_out = Joint()
        if nin == 2
            input_voltage = RealInput(nin=nin)
...
@mtkmodel DynamicsBias begin
    ...
    begin
        if single_pendulum
            τ_grav = dynamics_bias(MechanismState(mechanism, [q], [q̇]))[1]
        elseif hip
            τ_grav = dynamics_bias(MechanismState(mechanism, [q, other_joint.q], [q̇, other_joint.q̇]))[1]
        else # knee of double_pendulum
            τ_grav = dynamics_bias(MechanismState(mechanism, [other_joint.q, q], [other_joint.q̇, q̇]))[2]
        end
    end
    @equations begin
        q ~ joint_in.q
...
@mtkmodel FrictionTorque begin
    ...
    begin
        if stribeck
            # str_scale = τ_s#(τ_s - τ_c)
            # β = exp(-abs(q̇/q̇_s))
            τ_f = (τ_c - str_scale * β) * tanh(q̇ / 1e-4) + Kv * q̇
            # τ_f = tanh(q̇ / 1e-3) * τ_c + Kv * q̇
        else
            τ_f = tanh(q̇ / 1e-3) * τ_c + Kv * q̇
            # τ_f = Kv * q̇ + τ_c * sign(q̇)
        end
    end
