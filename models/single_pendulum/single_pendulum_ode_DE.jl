...
function pendulum_ode!(du, u, p, t)
    ...
    elseif fsm_state == MOVING_CW
        ...
        state = MechanismState(mechanism, [q], [ω])
...
function pendulum_conditions(out, u, t, integ)
    ...
    τ = -dynamics_bias(MechanismState(mechanism, [q], [ω]))[1]
...
function build_problem(mechanism_;u0 = [0., 0.0], tspan = (0.0, 120.0), torque_ref_="triangle")
    ...
    # v̇[1] = 0
    # τ = -inverse_dynamics(state, v̇)[1]
    τ = -dynamics_bias(MechanismState(mechanism, [u0[1]], [u0[2]]))[1]
    if τ > tau_s
