...
from src.plotting import (
    ...
    plot_data_q_dot, 
    plot_torques, 
    plot_computed_acceleration,
    plot_measured_U,
    plot_measured_I,
    plot_measured_q_dot
)
from src.found_parameters.parameters import PARAMETERS_TO_FIND
from src.objective_functions import model_error, objective_function_lite
import os
...
for file_name in os.listdir(folder_path):
    if file_name.endswith('.txt'):
        ...
        if df['DXL_Input_Voltage'].between(9.5, 16.0).all(): # else the collected data has errors
            ...
            # model_error(df, parameters, parameters_fixed, dt, with_current=False, verbose=True)
            objective_function_lite(df, parameters, parameters_fixed)
            # df.to_csv("output.csv")
            plot_torques(df, file_name)
            # plot_measured_U(df)
            # plot_measured_I(df)
            # plot_computed_acceleration(df, file_name)
            # plot_data_q(df)
            # plot_data_q_dot(df)
