...
using ModelingToolkit, Plots, DataFrames, RigidBodyDynamics, DifferentialEquations, ModelingToolkitStandardLibrary
using ModelingToolkit: t_nounits as t, D_nounits as D

include("../model_components.jl")

function system(mechanism_; pars = [0.06510461345450586, 1.5879662676966781, 0.39454422423683916, 0., 0.06510461345450586, 0.], stribeck=false, experiment = [1, 1], constant_input=false)
    ...
    # Todo : redefinition of v_input and output_reader -> should be a single component, not 2 separate ones
    # Also : OLController and position_sensor
...
function get_next_state(sys, u0, input, tspan)
    prob = ODEProblem(sys, [sys.J_total.q => u0[1], sys.J_total.q̇ => u0[2], sys.J_total2.q => u0[3], sys.J_total2.q̇ => u0[4]], (0., tspan), [sys.v_input.U => input[1], sys.v_input2.U => input[2]])
