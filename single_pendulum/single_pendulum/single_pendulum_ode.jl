...
function torque_ext(t; ref = "triangle")
    kt = 2.6657 #* 1.15
    Ra = 9.3756
...
function my_fun(t, τ_ref)
    return IfElse.ifelse(τ_ref == 0, torque_ext(t; ref = "triangle"), 
            IfElse.ifelse(τ_ref == 1, torque_ext(t; ref = "sawtooth"), torque_ext(t; ref = "square")))
end
...
@mtkmodel SP begin
    ...
    @equations begin
        τ ~ torque_ext(t; ref = τ_ref)
        D(θ) ~ ω
        D(ω) ~ fsm_state * (compute_dynamics(mechanism, θ, ω) + τ - sign_velocity * τ_c - (kt * ke / Ra + Kv) * ω) / (J_motor + mass_matrix(MechanismState(mechanism, [θ], [ω]))[1])
    end
    @continuous_events begin
        [ω ~ 0] => (affect!, [ω], [fsm_state, sign_velocity], [], 0)
    end
    @discrete_events begin
        ((fsm_state == 0) * ((compute_dynamics(mechanism, θ, ω) + torque_ext(t; ref = τ_ref)) > τ_s)) => (affect!, [], [fsm_state, sign_velocity], [], 1)
        ((fsm_state == 0) * ((compute_dynamics(mechanism, θ, ω) + torque_ext(t; ref = τ_ref)) < -τ_s)) => (affect!, [], [fsm_state, sign_velocity], [], 2)
    end
end
...
function plot_simulation(sol; torque_ref = "triangle")
    ...
    torques = [torque_ext(t; ref = torque_ref) for t in sol.t]
...
function plot_simu_vs_real(sol, df; torque_ref = "triangle")
    ...
    p0 = plot(sol.t, [torque_ext(t; ref = torque_ref) for t in sol.t], ylabel="    Torque [Nm]", label="Torque", color="green", 
            legendfontsize=12, tickfontsize=10, yguidefontrotation=0, linewidth=2)
