...
@mtkmodel FrictionTorque begin
    ...
    @continuous_events begin
        [q̇ ~ 0] => (affect!, [q̇], [fsm_state, sign_velocity], [], 0)
    end
    # @continuous_events begin
...
@mtkmodel SP begin
    ...
    @components begin
        ...
        τ_f = FrictionTorque(stribeck = stribeck, τ_c = 0.128, Kv = 1.2465, τ_s = 0.128, q̇_s = 0., fsm_state = fsm_state, sign_velocity = sign_velocity)
    ...
    @equations begin
        ...
        # connect(τ_grav.joint_out, J_total.joint_in)
    end
    @continuous_events begin
        [J_total.q̇ ~ 0] => (affect!, [J_total.q̇], [fsm_state, sign_velocity], [], 0)
    end
end

function build_problem(mechanism_, state_; u0 = [0., 0.0], change_tspan = false, tspan = (0.0, 120.0), pars = [0.128, 1.2465, 0.2843, 0.07305, 0.128, 0], experiment=1, stribeck=false)
    global state = state_
